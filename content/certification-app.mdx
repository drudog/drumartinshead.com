**Focus** — Full-Stack Design & Engineering · Learning Platform · 0→1
**Role** — Sole designer and engineer (end-to-end)
**Timeline** — 2025
**Live** — [partner-certification.vercel.app](https://partner-certification.vercel.app) · [github.com/drudog/partner-certification](https://github.com/drudog/partner-certification)
**Stack** — Next.js 16 · TypeScript · Tailwind CSS · shadcn/ui · Radix UI
...
Lesson content is written in Markdown and processed at request time through a `remark` → `rehype` → `remark-html` pipeline. HTML is injected via `dangerouslySetInnerHTML` on a server component — no JavaScript shipped for prose rendering. This replaced `next-mdx-remote` which bundled an incompatible React 18 copy against Next.js 16's React 19.
...
![Home — three tiered certification levels with progressive lock/unlock states and progress bars](/images/work/certification-app/01-home.png)

![Level overview — real-time progress tracking and module navigation](/images/work/certification-app/02-level.png)

![Module view — lessons with time estimates and quiz question count indicators](/images/work/certification-app/03-module.png)

![Lesson reader (desktop) — sticky sidebar navigation, formatted markdown content](/images/work/certification-app/04-lesson.png)
...
`next-mdx-remote` v6 bundled its own React 18 copy, throwing "React Element from an older version of React" errors against Next.js 16's React 19. Replaced the entire MDX pipeline with a lightweight `remark` + `remark-html` chain — no JSX emitted, no version conflict, smaller bundle.
